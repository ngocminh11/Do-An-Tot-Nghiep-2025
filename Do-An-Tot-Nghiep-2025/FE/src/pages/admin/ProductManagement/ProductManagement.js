...
const ProductManagement = () => {
  ...
  // Hàm lấy giá trị từ object theo đường dẫn
  ...
  const sortedProducts = useMemo(() => {
    ...
    return [...filteredProducts].sort((a, b) => {
      ...
      const aValue = getNestedValue(a, sorter.field);
      const bValue = getNestedValue(b, sorter.field);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (shortDescription) => {
        if (!shortDescription) return 'N/A';
        const words = shortDescription.split(/\s+/);
        if (words.length <= 10) return shortDescription;
  ...
  const handleExportExcel = async () => {
    try {
      // Gọi API backend để xuất file, truyền filter hiện tại
      const params = {
        name: filters.search,
        categoryId: filters.category,
      };
      const blob = await productService.exportProductsToExcel(params);
      // Xác định loại file (csv hoặc xlsx)
      const fileType = blob.type.includes('csv') ? 'csv' : 'xlsx';
      const fileName = fileType === 'csv' ? 'danh_sach_san_pham.csv' : 'danh_sach_san_pham.xlsx';
  ...
  return (
    <div className="product-management" style={{ maxWidth: 1200, margin: '0 auto', padding: '24px 0' }}>
      <div className="header" style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h1 style={{ margin: 0, color: '#1976d2', fontWeight: 700 }}>Quản lý sản phẩm</h1>
        <div style={{ display: 'flex', gap: 12 }}>
          <Button
            icon={<DownloadOutlined />}
            onClick={handleExportExcel}
            style={{ borderRadius: 24, background: '#e3f2fd', color: '#1976d2', fontWeight: 600, border: 'none', marginRight: 8 }}
          >
            Xuất Excel
          </Button>
          <Button type="primary" icon={<PlusOutlined />} onClick={() => navigate('/admin/products/add')} style={{ borderRadius: 24, background: '#1976d2', borderColor: '#1976d2', fontWeight: 600 }}>
            Thêm sản phẩm
          </Button>
        </div>
      </div>
      <div className="filters" style={{ marginBottom: 16, display: 'flex', gap: 16, flexWrap: 'wrap' }}>
        <Input placeholder="Tìm kiếm sản phẩm..." prefix={<SearchOutlined />} onChange={handleSearchChange} style={{ width: 250, borderRadius: 20, borderColor: '#e3f2fd' }} allowClear />
        <Select placeholder="Danh mục" allowClear style={{ width: 200, borderRadius: 20, borderColor: '#e3f2fd' }} onChange={handleCategoryChange} value={filters.category} disabled={!Array.isArray(categories) || categories.length === 0}>
      ...
      <Table
        ...
        scroll={{ x: 1300 }}
        style={{ borderRadius: 16, boxShadow: '0 4px 24px rgba(25,118,210,0.07)', border: '1.5px solid #e3f2fd' }}
      />
    </div>
  );
};
